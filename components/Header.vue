...
<template>
  <section class="bg-card border-b border-card-deep/30">
    <div class="w-full mx-auto md:px-12 px-8 max-w-7xl lg:px-16">
      <div class="relative flex flex-col w-full py-5 mx-auto md:items-center md:justify-between md:flex-row md:px-6">
        <div class="flex flex-row items-center justify-between lg:justify-start">
          <a href="/" class="text-foreground hover:text-accent inline-flex items-center gap-3">
          ...
          <button @click="handleOpen"
            class="inline-flex items-center justify-center p-2 text-foreground hover:accent focus:outline-none focus:text-primary md:hidden">
        ...
        <nav :class="{ hidden: !open, flex: open }" class="flex flex-col items-center flex-grow md:pb-0 md:flex md:justify-end md:flex-row lg:flex">
          <a v-for="menu in CONFIG.header.menus" class="px-2 lg:px-6 py-2 md:px-3 text-sm font-medium text-foreground hover:text-accent first:lg:ml-auto" :href="menu.href">{{$t(menu.text)}}</a>
